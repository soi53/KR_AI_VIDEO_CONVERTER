...
- [ ] **OpenAI Whisper 마이그레이션 검증**
  - 실제 환경에서 동작 확인
  - 자막 추출 정확도 비교 (Faster Whisper vs OpenAI Whisper)
  - 처리 속도 측정
  - 필요시 모델 사이즈 조정 (메모리 사용량 최적화)

- [ ] **메모리 최적화**
  - Whisper 모델 로드 시 메모리 사용량 모니터링
  - 대용량 동영상 처리 중 메모리 부족 문제 해결

- [ ] **에러 핸들링 강화**
  - 자막 추출 실패 시 상세 오류 메시지 제공
  - 자동 복구 메커니즘 구현

## 우선순위: 중간

- [ ] **성능 개선**
  - 대용량 파일 처리 시 지연 시간 단축
  - 일괄 처리 기능 추가 (여러 영상 순차 처리)

- [ ] **UI/UX 개선**
  - 진행 상황 표시 개선
  - 편의 기능 추가 (영상 길이, 오디오 채널 정보 등)
...
- [ ] **추가 기능**
  - 화면 속 텍스트 인식 및 번역 (OCR)
  - 다중 화자 구분
  - 자막 스타일 커스터마이징

## 완료된 항목

- [x] **Faster Whisper에서 OpenAI Whisper로 마이그레이션** (2025-04-14)
  - 업데이트된 Docker 설정 및 앱 코드
  - Python 버전 호환성 문제 해결
  - 라이브러리 의존성 정리

- [x] **영상 길이 제한 증가** (2024-03-12)
  - 최대 2GB 영상 지원
  - 프로세싱 타임아웃 조정

- [x] **음성 합성 엔진 업그레이드** (2024-03-12)
  - XTTS-v2 통합
  - 더 자연스러운 목소리 제공 